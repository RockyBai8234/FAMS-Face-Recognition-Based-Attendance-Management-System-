...
def manually_fill():
    ...
    def fill_attendance():
        ...
        csv_file_path = 'C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\StudentDetails\\StudentDetails.csv'
            ...
            def enter_data_DB():
                ...
                if ENROLLMENT == '' or STUDENT == '':
                    ...
                    time = datetime.datetime.fromtimestamp(
                        ts).strftime('%H:%M:%S')
                    row = [ENROLLMENT, STUDENT, Date, time]
                    try:
                        with open(csv_file_path, "a", newline="") as csv_file:
                            csv_writer = csv.writer(csv_file)
                            if csv_file.tell() == 0:
                                csv_writer.writerow(["ENROLLMENT", "NAME", "DATE", "TIME"])  # Write header if file is empty
                            csv_writer.writerow(row)
                        ENR_ENTRY.delete(first=0, last=22)
                        STUDENT_ENTRY.delete(first=0, last=22)
                        # Show notification
                        Notifi.configure(text="Data saved to CSV", bg="Green", fg="white")
                        Notifi.place(x=180, y=380)
                        MFW.update_idletasks()
                        MFW.after(2000, lambda: Notifi.configure(text=""))  # Clear notification after 2 seconds
                        # Open the CSV file
                        subprocess.Popen(['start', csv_file_path], shell=True)
                    except Exception as e:
                        print(f"Error saving data: {e}")

            Notifi = tk.Label(MFW, text="", bg="Green", fg="white", width=33,
    ...
    if l1 == '':
        ...
        try:
            ...
            detector = cv2.CascadeClassifier(
                'C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\haarcascade_frontalface_default.xml')
            Enrollment = txt.get()
            Name = txt2.get()
            sampleNum = 0
            while (True):
                ...
                for (x, y, w, h) in faces:
                    ...
                    cv2.imwrite("C:/Users/srina/OneDrive/Desktop/vscode/fams/TrainingImage/ " + Name + "." + Enrollment + '.' + str(sampleNum) + ".jpg",
                                gray)
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    recognizer.read("C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\TrainingImageLabel\\Trainner.yml")
                except:
                ...
                cs = 'C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'shiva':
            if password == 'shiva':
                ...
                cs = 'C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\StudentDetails\\StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
def collect_faces_and_ids():
    # Your logic to collect faces and IDs
    faces = []  # Populate this with your face data
    Id = []     # Populate this with corresponding IDs
    return faces, Id
...
def trainimg():
    ...
    global detector
    detector = cv2.CascadeClassifier("C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\haarcascade_frontalface_default.xml")
    try:
        global faces, Id
        faces, Id = getImagesAndLabels("C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\TrainingImage")
    except Exception as e:
        l = 'please make "C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\TrainingImage" folder & put Images'
    ...
    try:
        recognizer.save("C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\TrainingImageLabel\\Trainner.yml")
    except Exception as e:
        q = 'Please make "C:\\Users\\srina\\OneDrive\\Desktop\\vscode\\fams\\TrainingImageLabel" folder'
